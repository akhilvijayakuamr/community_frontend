...
import { UniquePostData, PostUpdateData, ReplyFormData } from '../../../utils/interface/user/post/postviewinterface'
...
export const PostView: React.FC = () => {
        ...
        try {
            ...
            setPostData((prevState) => ({
                ...
                comments: prevState.comments.map((comment) =>
                    comment.comment_id.toString() === commentReply.commentId
                        ? {
                            ...comment,
    ...
    const handleReportSubmit = async (postId: string) => {
        try {
            const response: AxiosResponse<string> = await postReport(postId, userId, report, headers);
            setIsPostOpen(!isPostOpen)
            setIsReportOpen(!isReportOpen)
    ...
    const handleCommentDelete = async (commentId: string) => {
        try {
            await CommentDelete(commentId, headers);
            setPostData((prevState) => ({
                ...prevState,
                comments: prevState.comments.filter(comment => comment.comment_id.toString() !== commentId),
                comment_count: prevState.comment_count - 1
            }))
        } catch {
    ...
    const handleReplayDelete = async (replyId: string, commentId: number) => {
        try {
            ...
            setPostData((prevState) => ({
                ...
                comments: prevState.comments.map((comment) =>
                    comment.comment_id === commentId
                        ? {
                            ...comment,
                            replies: comment.replies.filter(reply => reply.replay_id.toString() != replyId),
                            reply_count: comment.reply_count - 1
                        }
                        : comment
                )
                            ...
                            <section className="not-format">
                                ...
                                {postData.comments.map((data) =>
                                    <article key={data.comment_id} className="p-6 mb-6 text-base bg-white rounded-lg dark:bg-gray-900">
                                        ...
                                        {replyInputReplyListId === data.comment_id.toString() && (
                                            <div>
