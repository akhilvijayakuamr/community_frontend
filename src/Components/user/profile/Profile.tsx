...
import { UniquePostInterface , ProfileListUpdate, FollowerFollowingInterface, friends } from '../../../utils/interface/user/profile/profileinterface';
import { UserData, UserUpdate, getFriends, postLikeApi, userFollow } from '../../../Api/api';
...
export const Profile: React.FC = () => {
  ...
  const [profileData, setProfileData] = useState<UniquePostInterface>();
  ...
  const [followed, setFollowed] = useState<friends[]>([]);
  const [friends, setFriends] = useState<friends[]>([]);
  const [openFriends, setOpen] = useState<boolean>(false)
  ...
  const handleFollow = async (userId: string, followUserId: string) => {
    ...
    const updatedPost:any = {
      ...
      followers_count: profileData?.follow 
        ? (profileData.followers_count ?? 0) - 1 
        : (profileData?.followers_count ?? 0) + 1
    };
  ...
  useEffect(() => {
    ...
    const profileDataCall = async () => {
      try {
        const response: AxiosResponse<UniquePostInterface> = await UserData(userId, profileId, headers);
  ...
  const getAllFriends = async (id: string|undefined, opration: string) => {
    try {
      const response: AxiosResponse<FollowerFollowingInterface> = await getFriends(id??'', headers)
      setFollowers(response.data.follower)
      setFollowed(response.data.followed)
      if (opration === "followers") {
        setFriends(response.data.follower)
      }
      else {
        setFriends(response.data.followed)
      }
  ...
  // Change followers and followed

  const displyFriends = (type: string) => {
    if (type === "Followers") {
                    ...
                    <div className="flex flex-wrap justify-center">
                      ...
                      <div className="w-full lg:w-4/12 px-4 lg:order-1">
                        <div className="flex justify-center py-4 lg:pt-4 pt-8" >
                          <div className="mr-4 p-3 text-center cursor-pointer" onClick={() => getAllFriends(profileData?.id, "followers")}>
                          ...
                          <div className="lg:mr-4 p-3 text-center cursor-pointer" onClick={() => getAllFriends(profileData?.id, "following")}>
                            <span className="text-xl font-bold block uppercase tracking-wide text-gray-300">
              ...
              <div className="p-4 md:p-5 border-b rounded-t dark:border-gray-600">
                <ul>
                  {friends.map((user) => (
                    <li
                      key={user.id}
                      onClick={() => handleGetProfile(user.id.toString())}
                      className="flex items-center bg-gray-600 p-3 border-b hover:bg-gray-500 cursor-pointer last:border-b-0"
                    >
                      {user.user_profile ? (
                        <img
